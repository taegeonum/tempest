...
public class NaiveTopologyBuilder implements AppTopologyBuilder {
  ...
  public NaiveTopologyBuilder(BaseRichSpout spout,
    ...
    for (Timescale ts : timescales) {
      ...
      String counterId = "NaiveWindowOperator-(w=" + ts.windowSize + ", i=" + ts.intervalSize + ")";
      builder.setBolt(counterId, ij.getInstance(WordCountByWindowBolt.class), numBolt)
      .fieldsGrouping(spoutId, new Fields("word"));
      String rankerId = "(w=" + ts.windowSize + ", i=" + ts.intervalSize + ")";
      builder.setBolt(rankerId, rankingBolt, 1).allGrouping(counterId);
      i += 1;
    }
