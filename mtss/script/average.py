...
path = args.path
cpu_file = open(path + "cpu")
mem_file = open(path + "memory")
avg_cpu = []
avg_mem = []

lines = [line.strip() for line in cpu_file]
for line in lines:
  cpu = float(line.split()[1])
  avg_cpu.append(cpu)

lines2 = [line.strip() for line in mem_file]
for line in lines2:
  mem = float(line.split()[1])
  avg_mem.append(mem)

print "path: ", path
print "avg cpu: ", np.average(avg_cpu)
print "avg mem: ", np.average(avg_mem)
