 ...
 */
public final class DynamicDPTradeOffLookupTableImpl<V> implements DynamicOutputLookupTable<V> {
  private static final Logger LOG = Logger.getLogger(DynamicDPTradeOffLookupTableImpl.class.getName());
  ...
  @Inject
  private DynamicDPTradeOffLookupTableImpl() {
  ...
  public void saveOutput(final long startTime, final long endTime, final Timescale timescale, final V output) {
    ...
    if (row == null) {
      table.putIfAbsent(startTime, new ConcurrentSkipListMap<Long, ConcurrentMap<Timescale, V>>());
  ...
  public WindowTimeAndOutput<V> lookupLargestSizeOutput(final long startTime, final long endTime)
    ...
    if (largestKey == null) {
      ...
      final Map<Timescale, V> map = row.get(largestKey);
      if (map == null) {
        throw new NotFoundException(
            "Cannot lookup endTime " + endTime  + "from " + startTime + " in lookupLargestSizeOutput");
      }
      final V firstV = map.values().iterator().next();
      return new WindowTimeAndOutput<V>(startTime, largestKey, firstV);
    }
  }
