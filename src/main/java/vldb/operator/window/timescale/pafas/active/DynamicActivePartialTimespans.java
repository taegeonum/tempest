 ...
 */
public final class DynamicActivePartialTimespans<T> implements DynamicPartialTimespans<T> {
  private static final Logger LOG = Logger.getLogger(DynamicActivePartialTimespans.class.getName());
  ...
  private long lastSlice;
  /**
   * StaticComputationReuserImpl Implementation.
   * @param startTime an initial start time
   */
  @Inject
  private DynamicActivePartialTimespans(final WindowManager windowManager,
    ...
    this.rebuildSize = windowManager.getRebuildSize() + 10;
    this.currIndex = startTime + rebuildSize;
    this.windowManager = windowManager;
    this.activeSlices = new HashSet<>();
    this.lastSlice = startTime;
  ...
  private boolean isActiveSlice(final long time) {
    for (final Timescale timescale : timescales) {
      ...
      long tsStart = st - (timescale.windowSize - timescale.intervalSize);
  ...
  public boolean isSlicable(final long time) {
    return partialTimespanMap.containsKey(time) || lastSlice == time;
  }

  public boolean isActive(final long time) {
    return activeSlices.contains(time);
  }
  ...
  public void removeNode(final long startTime) {
    final Node node = partialTimespanMap.remove(startTime);
    for (long i = node.start; i < node.end; i++) {
      activeSlices.remove(i);
    }
  }
