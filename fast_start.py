...
parser.add_argument("-tradeoff_factor", type=float, default=100000.0)
...
cmd = "java -cp %s %s --test_name=%s --variable=%s --timescales=\"%s\" --data_path=%s --input_rate=%d --total_time=%d --num_threads=%d --output_path=%s --operator_type=%s --tf=%f" % (classpath, target_class, args.testname, args.variable, timescales, args.data_path, args.input_rate, args.total_time, args.num_threads, args.output_path, args.operator_type, args.tradeoff_factor)
