...
import edu.snu.tempest.operator.window.common.StaticTSOutputGenerator;
...
public class StaticSlicedWindowOperatorTest {
  StaticTSOutputGenerator<Map<Integer, Integer>> tsOutputGenerator;
  ...
  public void initialize() {
    tsOutputGenerator = mock(StaticTSOutputGenerator.class);
  ...
  public void defaultSlicedWindowTest() {
    final StaticSlicedWindowOperatorImpl<Integer, Map<Integer, Integer>> operator =
        new StaticSlicedWindowOperatorImpl<>(aggregator, tsOutputGenerator, 0L);

    when(tsOutputGenerator.nextSliceTime()).thenReturn(1L, 3L, 4L, 6L, 7L, 9L, 10L, 12L);
    ...
    operator.onNext(1L);
    verify(tsOutputGenerator).savePartialOutput(0, 1, result);
    ...
    operator.onNext(3L);
    verify(tsOutputGenerator).savePartialOutput(1, 3, result2);
    ...
    operator.onNext(4L);
    verify(tsOutputGenerator).savePartialOutput(3, 4, result3);

    operator.onNext(6L);
    verify(tsOutputGenerator).savePartialOutput(4, 6, new HashMap<Integer, Integer>());
  }
